...
function doGet() {
  return jsonResponse_({
    ok: true,
    service: "house190-drive-uploads",
    storage: "google-drive",
    actions: ["uploadClockSelfie", "uploadClosingAttachment", "deleteClosingAttachment"],
  });
}
...
function doPost(event) {
  ...
  try {
    ...
    if (action === "uploadClockSelfie") return uploadClockSelfie_(payload);
    if (action === "uploadClosingAttachment") return uploadClosingAttachment_(payload);
    if (action === "deleteClosingAttachment") return deleteClosingAttachment_(payload);
...
function uploadClockSelfie_(payload) {
  ...
  const file = existing.hasNext()
    ? existing.next()
    : destination.createFile(Utilities.newBlob(image, "image/jpeg", fileName));
...
function uploadClosingAttachment_(payload) {
  ...
  assert_(/^[A-Za-z0-9-]{16,100}$/.test(requestId), "Identificador do arquivo inválido.");
  assert_(/^[A-Za-z0-9_-]{8,180}$/.test(closingId), "Fechamento inválido.");
  ...
  const file = existing.hasNext()
    ? existing.next()
    : destination.createFile(Utilities.newBlob(decoded.bytes, decoded.mimeType, fileName));
  file.setDescription(JSON.stringify({
...
function deleteClosingAttachment_(payload) {
  ...
  assert_(metadata.origem === "Fechamento de Caixa House190", "Arquivo não autorizado.");
  const privileged = profile.role === "admin" || profile.role === "finance";
  assert_(privileged || String(metadata.uploaderUid || "") === uid, "Sem permissão para excluir.");
...
function firebaseProfile_(uid, token) {
  ...
  assert_(response.getResponseCode() === 200, "Sessão expirada ou sem permissão.");
  const profile = JSON.parse(response.getContentText() || "null");
  assert_(profile && String(profile.UsuarioID || "") === uid, "Perfil não autorizado.");
  return profile;
}
...
function decodeSelfie_(dataUrl) {
  ...
  assert_(bytes.length <= SELFIE_SERVICE_CONFIG_.maxImageBytes, "A selfie ultrapassa o limite permitido.");
  return bytes;
}
...
function decodeClosingFile_(dataUrl, informedMimeType) {
  ...
  assert_(allowed.indexOf(mimeType) >= 0 && match[1].toLowerCase() === mimeType, "Formato não permitido.");
  const bytes = Utilities.base64Decode(match[2].replace(/\s/g, ""));
  assert_(bytes.length > 0, "O comprovante está vazio.");
  assert_(bytes.length <= CLOSING_SERVICE_CONFIG_.maxFileBytes, "O comprovante ultrapassa 2 MB.");
...
function destinationFolder_(profile, day) {
  ...
  return childFolder_(month, safeName_(profile.NomeLoja || profile.LojaID || "Sem loja"));
}
...
function closingDestinationFolder_(store, day) {
  ...
  const month = childFolder_(year, day.slice(0, 7));
  return childFolder_(month, safeName_(store));
}
